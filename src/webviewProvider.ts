...
import { Task, TaskStatus, WebviewPayload, TaskViewData, WebviewMessage, POOL_LABELS, POOL_COLORS, ApprovalRequest } from './types';
...
export class WebviewProvider implements vscode.WebviewViewProvider, vscode.Disposable {
    ...
    constructor(
        ...
        private getSchedulerState: () => { runningModules: string[]; active: boolean; pendingApproval: ApprovalRequest | null }
    ) {}

    setMessageHandler(handler: (msg: WebviewMessage) => void): void {
    ...
    resolveWebviewView(view: vscode.WebviewView): void {
        ...
        view.webview.onDidReceiveMessage(msg => this.messageHandler?.(msg as WebviewMessage));
    }
    ...
    openDashboard(): void {
        ...
        this.panel = vscode.window.createWebviewPanel(
            'aiTaskPipeline.dashboard',
            'Task_Pipeline.dashboard',
            vscode.ViewColumn.One,
            { enableScripts: true, retainContextWhenHidden: true }
        );
        this.panel.webview.html = this.getDashboardHtml(this.panel.webview);
        this.panel.webview.onDidReceiveMessage(msg => this.messageHandler?.(msg as WebviewMessage));
    ...
    private buildPayload(): WebviewPayload {
        const state = this.getSchedulerState();
        const tasks = this.taskManager.getAllTasks();
        const taskViews: TaskViewData[] = tasks.map(task => {
            ...
            return {
                ...
                hasConflict: task.status === TaskStatus.Todo && conflictingModules.length > 0,
                conflictingModules,
                isProcessing: task.status === TaskStatus.InProgress,
...
<div id="app">
  ...
  <div class="board" id="board">
    <div class="column" data-status="todo">
      <div class="col-header"><span class="col-dot" style="background:${POOL_COLORS[TaskStatus.Todo]}"></span><span class="col-title">${POOL_LABELS[TaskStatus.Todo]}</span><span class="col-count" id="count-todo">0</span></div>
      <div class="col-cards" id="col-todo"></div>
    </div>
    <div class="column col-active" data-status="inProgress">
      <div class="col-header"><span class="col-dot" style="background:${POOL_COLORS[TaskStatus.InProgress]}"></span><span class="col-title">${POOL_LABELS[TaskStatus.InProgress]}</span><span class="col-count" id="count-inProgress">0</span></div>
      <div class="col-cards" id="col-inProgress"></div>
    </div>
    <div class="column" data-status="pendingReview">
      <div class="col-header"><span class="col-dot" style="background:${POOL_COLORS[TaskStatus.PendingReview]}"></span><span class="col-title">${POOL_LABELS[TaskStatus.PendingReview]}</span><span class="col-count" id="count-pendingReview">0</span></div>
      <div class="col-cards" id="col-pendingReview"></div>
    </div>
    <div class="column" data-status="completed">
      <div class="col-header"><span class="col-dot" style="background:${POOL_COLORS[TaskStatus.Completed]}"></span><span class="col-title">${POOL_LABELS[TaskStatus.Completed]}</span><span class="col-count" id="count-completed">0</span></div>
      <div class="col-cards" id="col-completed"></div>
    </div>
    <div class="column" data-status="terminated">
      <div class="col-header"><span class="col-dot" style="background:${POOL_COLORS[TaskStatus.Terminated]}"></span><span class="col-title">${POOL_LABELS[TaskStatus.Terminated]}</span><span class="col-count" id="count-terminated">0</span></div>
