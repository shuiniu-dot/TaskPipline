...
class AiService {
    ...
    async aiExtractMetadata(model, task) {
        const systemMsg = vscode.LanguageModelChatMessage.Assistant('You are a code analysis assistant. Given a task title and description, predict which modules or file areas of the codebase this task will modify. Return ONLY a JSON array of short module name strings (lowercase, no spaces). Example: ["auth","user_db","middleware"]');
        const userMsg = vscode.LanguageModelChatMessage.User(`Task title: ${task.title}\nTask description: ${task.description}\n\nReturn the JSON array:`);
     ...
     */
    async processTaskWithTools(task, onProgress, onApproval, onOperation) {
        const model = await this.getModel();
        if (!model) {
        ...
        for (let round = 0; round < maxRounds; round++) {
            onProgress?.(Math.min(85, 10 + round * 4), `Agent round ${round + 1}: analyzing and working...`);
            let response;
            try {
            ...
            for (const toolCall of toolCallParts) {
                ...
                if (operation.isHighRisk) {
                    ...
                    if (!approved) {
                        ...
                        resultParts.push(new vscode.LanguageModelToolResultPart(toolCall.callId, [new vscode.LanguageModelTextPart('Operation rejected by user. Skip this operation and continue with an alternative approach.')]));
                        continue;
                    }
                ...
                // Execute the tool
                try {
                    const result = await this.executeTool(toolCall.name, toolCall.input, operation);
                    if (operation.status !== 'approved') {
                        operation.status = 'success';
                    }
                    operation.output = result.substring(0, 3000);
                    resultParts.push(new vscode.LanguageModelToolResultPart(toolCall.callId, [new vscode.LanguageModelTextPart(result)]));
                }
                catch (err) {
                    operation.status = 'error';
                    operation.output = String(err).substring(0, 1000);
                    resultParts.push(new vscode.LanguageModelToolResultPart(toolCall.callId, [new vscode.LanguageModelTextPart(`Error: ${err}`)]));
                }
                onOperation(operation);
            }
    ...
    /* ======================================================================
       Tool Execution
       ====================================================================== */
    async executeTool(name, input, operation) {
    ...
    /* ======================================================================
       ...
       ====================================================================== */
    createOperation(taskId, toolCall) {
        const input = toolCall.input;
        const type = toolCall.name;
        ...
        return {
            ...
            timestamp: Date.now(),
            status: 'pending',
            isHighRisk: types_1.HIGH_RISK_OPERATIONS.has(type),
        };
    }
    getOperationTarget(type, input) {
        switch (type) {
            case 'readFile':
            case 'writeFile':
            case 'editFile':
    ...
    summarizeOperations(operations) {
        ...
        if (rejected > 0) {
            lines.push(`  - Operations rejected: ${rejected}`);
        }
        if (errors > 0) {
            lines.push(`  - Operations failed: ${errors}`);
        }
        return lines.join('\n');
    }
    /* ======================================================================
       ...
       ====================================================================== */
    async processTask(task, onProgress) {
        const model = await this.getModel();
        if (model) {
            try {
    ...
    async aiProcessTask(model, task, onProgress) {
        onProgress?.(10, 'Analyzing task requirements...');
        const systemMsg = vscode.LanguageModelChatMessage.Assistant('You are a code generation assistant. Process the given development task and provide a concise summary of the changes you would make. Be specific about what files would be modified and what the changes would be. Keep your response under 200 words.');
        const userMsg = vscode.LanguageModelChatMessage.User(`Task: ${task.title}\nDescription: ${task.description}\nPredicted modules: ${task.modules.join(', ')}\n\nProvide a summary of the implementation:`);
    ...
    async simulateProcessTask(task, onProgress) {
        const steps = [
    ...
    buildInitialMessages(task) {
        const system = vscode.LanguageModelChatMessage.Assistant('You are an autonomous code agent working inside VS Code. You have tools to read files, write files, edit files, search code, list directories, and run commands. ' +
        ...
        const user = vscode.LanguageModelChatMessage.User(`Please implement the following development task by using the available tools:\n\n` +
            `## Task: ${task.title}\n` +
            `Description: ${task.description}\n` +
